...
Как 
    пользователь
я хочу 
    иметь возможность посмотреть всю историю своих путешествий с различными аналитиками: стоимости (в рахзных разрезах)/ длительности и тп
чтобы 
...
3. Система должна позволять хранить данные 20000 пользователей в этом году из расчета на 2 поездки в год на 1 пользователя, где предполагается 2 перелета, 1 бронирование отеля, 1 бронирование автомобиля
...
Как вариант использовать лямбда архитектуру. 
Лямбда архитектура позволяет организовывать хранилища данных и потоки управления ими таким образом что данные из приложений, где допустима 
асинхронная батчевая загрузка - отправляются в медленный слой с субд специализированной для аналитических задач и есть допонительный слой данных, которые догружаются напрямую в клиент для анализа. 
